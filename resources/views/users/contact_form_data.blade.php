                    ...
                    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                        ...
                        <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                            ...
                            <h1 class="d-flex align-items-center text-dark fw-bolder fs-3 my-1">Contacts
                                ...
                                <!--end::Separator-->
                                <!--begin::Description-->
                                <small class="text-muted fs-7 fw-bold my-1 ms-1">Data List</small>
                                <!--end::Description--></h1>
                            <!--end::Title-->
                        </div>
                        <!--end::Page title-->
		...
		<div class="card mt-4">
			...
			<div class="d-flex justify-content-between align-items-start card-header border-0 p-1">
				<h3 class="card-title align-items-start flex-column">
					<span class="card-label fw-bolder fs-3 mb-1">Contcat Data List</span>
					<!-- <span class="text-muted mt-1 fw-bold fs-7">Leads Form data here</span> -->
				</h3>

				<div class="d-flex flex-wrap gap-2">
				<form action="{{ route('contact-form-data') }}" method="GET" class="d-flex">
					<!--begin::Input group-->
					...
					<a href="{{ route('contact-form-data') }}" class="btn btn-danger btn-sm">Refresh</a>
				</form>
			</div>
				...
				@if($result->isNotEmpty())
					...
					<table class="table table-sm table-condensed table-row-bordered table-row-gray-100 align-middle gs-0 gy-3">
						...
						<tr class="fw-bolder text-muted bg-light bd-cyan">
							...
							<th class="min-w-110px">Full Name</th>
							<th class="min-w-110px">Email</th>
							<th class="min-w-110px">Phone</th>
							<th class="min-w-110px">Subject</th>
							<th class="min-w-110px">Message</th>
						...
						<tr>
							<td class="ps-5 text-dark fs-6">{{($result->currentPage() - 1) * $result->perPage() + $loop->iteration}}</td>
							<td class="text-dark fs-6">{{ $data->full_name }}</td>
							<td class="text-dark fs-6">{{ @$data->email }}</td>
							<td class="text-dark fs-6">{{ $data->phone }}</td>
							<td class="text-dark fs-6">{{ $data->subject }}</td>
							<td class="text-dark fs-6">{{ $data->contact_message }}</td>
							<td>
								<div class="d-inline-flex justify-content-end gap-1 w-100 border-bottom-0">
									<form action="{{ route('delete-contact-message', $data->id) }}" method="POST" style="display: inline;">
										@csrf
										@method('DELETE')
...
<script>
    function confirmDelete() {
        if (confirm("Are you sure you want to delete this contact data?")) {
